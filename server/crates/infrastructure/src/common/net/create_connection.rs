...
pub async fn create_connection(
    port: u16,
    config: NetConfig,
) -> Result<(TlsAcceptor, TcpListener), NetError> {
    ...
    let server_cert_path = format!("{}/server.crt", config.cert_dir);
    let key_path = format!("{}/server.key", config.cert_dir);
    let client_crt_path = format!("{}/client.crt", config.cert_dir);

    generate_certs(
        server_cert_path.clone(),
        key_path.clone(),
        client_crt_path,
        config.binds.iter().map(|e| e.to_string()).collect(),
    )
